...
Our methods followed prepared plans, which we collect into a protocol in the Appendix.
We also note deviations from these plans and the reasons for them.
...
googlesheets4::read_sheet(
  ...
  rename_with(snakecase::to_snake_case) |>
  filter(is.na(synthesis)) |> 
  # mutate(zotero_key = str_split(zotero_key, "; ")) |> 
  # tidyr::unnest(zotero_key) |> 
  transmute(
    # Citation = paste0("@", zotero_key),
    Citation = sapply(
      str_split(zotero_key, "; "),
      function(s) str_c("[", str_c("@", s, collapse = "; "), "]")
    ),
...
Following discussion among AC, PMJ, and JCB, we omitted results from Google Scholar due to missing abstracts, missing URLs, high overlap with other search results, and irreproducibility of the search process.
